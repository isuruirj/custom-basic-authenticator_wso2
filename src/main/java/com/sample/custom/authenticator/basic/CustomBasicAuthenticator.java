...
public class CustomBasicAuthenticator extends BasicAuthenticator {
    ...
    protected void processAuthenticationResponse(HttpServletRequest request,
        ...
        if (queryParams.contains("scope=")) {
            String scopes = queryParams.substring(queryParams.indexOf("scope=")+6);
            if (scopes.contains("&")){
                scopes = scopes.substring(0,scopes.indexOf("&"));
            }
            ...
            for(String scope : scopeList) {
                if (scope.startsWith("US_")) {
                    ...
                    if (log.isDebugEnabled()) {
                        log.debug("Username: " + username + ", User store domain: " + userstoreDomain);
                    }
                    break;
                }
            }
